...
adapterAllow(X):- char_code("A",Low),char_code("H",High), char_code(X, Var),  range(Low, High, Var).
...
dotted_IP_address_to_int([Inhead|Tail],[Expval|Tail1], Sum):- dotted_IP_address_to_int(Tail ,Tail1, Sum1),atom_number(Inhead, Remtail), atom_number(Expval, Exp1),Mult is 256**Exp1, Sum is (Sum1 + (Remtail * Mult)).

% This function checks the range for the IP Address between certain
% limts that can be set by the user.
ip_range(K):- split_string(K,".","",SplitListIP),split_string("192.168.10.0",".","",SplitLowerIP),split_string("192.168.10.255",".","",SplitUpperIP),dotted_IP_address_to_int(SplitListIP,["3", "2", "1", "0"],ListIPInt),dotted_IP_address_to_int(SplitLowerIP,["3", "2", "1", "0"],LowerIPInt),dotted_IP_address_to_int(SplitUpperIP,["3", "2", "1", "0"],UpperIPInt),range(LowerIPInt,UpperIPInt,ListIPInt).

% The following code blocks specific IP address
% by returning a false for them in case they are source
ipv4src(X):- ((X="198.2.18.64" ; X="198.4.33.12") -> false;true),(ip_range(X) -> false;true).

% The following code blocks specific IP address
% by returning a false for them in case they are destination
ipv4dst(Y):- ((Y="173.6.2.33" ; Y="45.7.43.12") -> false;true),(ip_range(Y) -> false;true).
